...
function generatePlane() {
  ...
  for (let i = 0; i < planeMesh.geometry.attributes.position.count; i++) {
    colors.push(0, 0.01, 0.4) // change the color
  }
...
function animate() {
  ...
  raycaster.setFromCamera(mouse, camera) // intersects hover effect
  frame += 0.01
  ...
  for (let i = 0; i < array.length; i += 3) {
    ...
    // y
    array[i + 1] = originalPosition[i] + Math.sin(frame + randomValues[i + 1]) * 0.001
  }
  ...
  if (intersects.length > 0) {
    ...
    // COLOR FADE HOVER EFFECT (GSAP library)
    const initialColor = {
      r: 0,
      g: 0.19,
      b: 0.4
...
// ADD MOUSEMOVE EVENTLISTENER & NORMALIZE MOUSE COORDINATES
...
document.querySelector('#viewWorkBtn').addEventListener('click', (e) => {
  ...
  gsap.to(camera.position, {
    ...
    delay: 1.2,
    onComplete: () => {
      window.location = 'https://www.yunbin-bae.com/'
    }
  })
})
